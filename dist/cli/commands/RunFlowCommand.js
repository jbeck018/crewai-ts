...
export class RunFlowCommand extends Command {
    ...
    syntax = '<flow-file-path> [--input <input-json>] [options]';
    examples = [
     ...
     */
    setupVerboseTracking(flow) {
        // Subscribe to flow events for verbose tracking
        flow.events.on('flow_started', (event) => {
        ...
        flow.events.on('method_execution_failed', (event) => {
...
//# sourceMappingURL=RunFlowCommand.js.map