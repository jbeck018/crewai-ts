...
export class ContextualMemory {
     ...
     */
    async searchByMetadata(metadata, limit = 10) {
        // Start performance tracking
        const startTime = Date.now();
     ...
     */
    removeFromMetadataIndex(id, metadata) {
        // Process each metadata key-value pair
        for (const [key, value] of Object.entries(metadata)) {
            // Convert value to string for indexing
            const valueStr = String(value);
     ...
     */
    findItemsMatchingMetadata(metadata) {
    ...
    updateQueryMetrics(startTime) {
        ...
        if (totalQueries === 1) {
            ...
            this.metrics.averageQueryTimeMs = (this.metrics.averageQueryTimeMs * (totalQueries - 1) + queryTime) / totalQueries;
        }
    }
    /**
...
export class VectorStoreMemory {
     ...
     */
    async add(id, vector, content, metadata = {}) {
     ...
     */
    async similaritySearch(queryVector, limit = 5, scoreThreshold = 0.7) {
        // Start performance tracking
        const startTime = Date.now();
