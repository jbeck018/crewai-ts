    ...
    async complete(messages, options = {}) {
        ...
        if (!options.streaming && this.cache.has(cacheKey)) {
            if (this.enableLogging)
                console.log('u2713 Using cached LLM response');
        ...
        try {
            ...
            const response = await this.executeWithRetries(`${this.baseUrl}/messages`, {
                method: 'POST',
    ...
    async executeWithRetries(url, init, attempt = 1) {
        try {
            ...
            if (retryable && attempt <= this.maxRetries) {
                ...
                const delay = Math.min(this.retryDelay * Math.pow(2, attempt - 1) + Math.random() * 200, 15000 // Max 15s delay (Claude can be slower)
                );
                if (this.enableLogging) {
            ...
            if (error instanceof Error && error.name === 'TimeoutError' && attempt <= this.maxRetries) {
                const delay = Math.min(this.retryDelay * Math.pow(2, attempt - 1) + Math.random() * 200, 15000 // Max 15s delay
                );
                if (this.enableLogging) {
...
//# sourceMappingURL=AnthropicLLM.js.map