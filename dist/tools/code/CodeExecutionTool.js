 ...
 */
export function createCodeExecutionTool(config = {}) {
    const { unsafeMode = false, defaultTimeout = 30000, workingDirectory = os.tmpdir(), validateDocker = true, useDocker = false } = config;
    ...
    return createStructuredTool({
        ...
        func: async ({ code, language, timeout = defaultTimeout }) => {
            ...
            try {
                ...
                const tempFilePath = path.join(workingDirectory, `code_exec_${execId}.${langConfig.extension}`);
 ...
 */
function executeLocally(filePath, langConfig, timeout) {
    try {
 ...
 */
function executeInDocker(filePath, langConfig, timeout) {
    try {
...
//# sourceMappingURL=CodeExecutionTool.js.map