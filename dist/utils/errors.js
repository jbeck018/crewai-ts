    ...
    toJSON() {
        return {
            ...
            'toJSON' in this.cause && typeof this.cause.toJSON === 'function'
                ? this.cause.toJSON()
                : { message: this.cause.message, stack: this.cause.stack }) : undefined
        };
    }
    /**
...
export async function withRetry(fn, options = {}) {
    ...
    const retryPredicate = options.retryPredicate ?? ((error) => {
    ...
    while (true) {
        try {
            ...
            const backoffMs = Math.min(maxDelayMs, initialDelayMs * Math.pow(backoffFactor, attempt - 1));
