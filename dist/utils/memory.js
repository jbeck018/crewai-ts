...
export class MemoryManager {
    ...
    async updateMemory(id, updates) {
        ...
        if (updates.type !== undefined)
            updatedMemory.type = updates.type;
        if (updates.importance !== undefined)
            updatedMemory.importance = updates.importance;
        if (updates.metadata !== undefined)
            updatedMemory.metadata = updates.metadata;
        if (updates.tags !== undefined)
            updatedMemory.tags = updates.tags;
        if (updates.source !== undefined)
            updatedMemory.source = updates.source;
     ...
     * @returns The memory entry or null if not found
     */
    async getMemory(id, trackAccess = true) {
        try {
    ...
    async semanticSearch(query, options = {}) {
        ...
        try {
            ...
            const results = memories.map(memory => ({
                memory,
                score: similarity.cosine(toNumberArray(queryEmbedding), toNumberArray(memory.embedding))
            }));
