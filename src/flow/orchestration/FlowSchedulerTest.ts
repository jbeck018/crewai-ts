...
export class TestFlowScheduler extends EventEmitter {
  ...
  private updateResourceUsage(flow: SchedulableFlow, isStarting: boolean): void {
    ...
    if (isStarting) {
      this.resourceUsage.cpu += cpu;
      this.resourceUsage.memory += memory;
      this.resourceUsage.io += io;
      this.resourceUsage.network += network;
    } else {
