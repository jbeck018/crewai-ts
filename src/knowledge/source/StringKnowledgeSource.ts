...
export class StringKnowledgeSource extends BaseKnowledgeSource {
   ...
   * @private
   */
  protected override createChunk(content: string): KnowledgeChunk {
    return {
   ...
   * @private
   */
  protected override generateChunkId(content: string): string {
   ...
   * @private
   */
  protected override splitIntoChunks(text: string): string[] {
   ...
   * @private
   */
  private recursiveChunkingOptimized(text: string): string[] {
   ...
   * @private
   */
  private semanticChunkingOptimized(text: string): string[] {
    const { chunkSize, chunkOverlap: overlap } = this.options;
    const content = text;
   ...
   * @private
   */
  private hybridChunkingOptimized(text: string): string[] {
    const { chunkSize, chunkOverlap: overlap } = this.options;
    const content = text;
