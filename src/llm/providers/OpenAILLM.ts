...
export class OpenAILLM implements BaseLLM {
  ...
  async complete(messages: LLMMessage[], options: Partial<LLMOptions> = {}): Promise<LLMResult> {
    ...
    if (!options.streaming && this.cache.has(cacheKey)) {
      if (this.enableLogging) console.log('✓ Using cached LLM response');
    ...
    if (actualMaxTokens <= 0) {
      throw new Error(
        `Prompt exceeds maximum token limit for model ${modelName}. ` +
    ...
    try {
      ...
      const response = await this.executeWithRetries<OpenAIResponse>(
