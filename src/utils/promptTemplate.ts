...
const templateCache = new Cache<CompiledTemplate>({
  maxSize: 100,
  ttl: 3600000 // 1 hour
});
 ...
 */
type CompiledTemplate = (variables: TemplateVariables) => string;
...
export class PromptTemplate {
  ...
  private compiled: CompiledTemplate | null = null;
   ...
   */
  async render(variables: TemplateVariables = {}): Promise<string> {
      ...
      if (tokens > this.options.maxTokens) {
        ...
        if (!this.options.autoTruncate || !this.truncationStrategy) {
          throw new ValidationError(
            `Rendered template exceeds token limit: ${tokens}/${this.options.maxTokens}`,
            { context: { template: this.template.substring(0, 100) + '...' } }
          );
        }
  ...
  private compileTemplate(template: string): CompiledTemplate {
    ...
    return (vars: TemplateVariables): string => {
      let result = template;
   ...
   */
  private async truncateVariables(variables: TemplateVariables): Promise<TemplateVariables> {
    if (!this.truncationStrategy || !this.options.maxTokens) {
      return variables;
    }
    
    // Get variable token counts
    const tokenCounts = new Map<string, TokenCountResult>();
    let totalTokens = 0;
    
    for (const [key, value] of Object.entries(variables)) {
      if (typeof value === 'string' && value.length > 0) {
        const tokenCount = tokenCounter.countTokens(value, this.options.model as any);
    ...
    const templateOverhead = tokenCounter.countTokens(
      emptyRendered,
      this.options.model as any
    ).tokens;
    ...
    if (remainingTokens > 0) {
      ...
      for (const key of proportionVars) {
        ...
        const additionalAllocation = Math.min(
          Math.max(0, targetAllocation - currentAllocation),
          remainingTokens,
          currentCount - currentAllocation
        );
    ...
    for (const [key, allocation] of allocations.entries()) {
      ...
      if (allocation < currentCount) {
        ...
        if (this.truncationStrategy?.customTruncate?.[key]) {
          truncatedVars[key] = this.truncationStrategy.customTruncate[key](
            variables[key] as string,
            allocation
          );
        } else {
          // Default truncation: keep beginning and end, drop middle
          truncatedVars[key] = this.truncateText(
            variables[key] as string,
            allocation,
            this.options.model
          );
   ...
   */
  private truncateText(text: string, maxTokens: number, model?: string): string {
    if (!text) return text;
    
    const { tokens } = tokenCounter.countTokens(text, model as any);
    ...
    if (maxTokens < 10) {
      ...
      while (tokenCounter.countTokens(truncated, model as any).tokens > maxTokens) {
    ...
    while (tokenCounter.countTokens(frontText, model as any).tokens > frontTokens) {
      frontText = frontText.slice(0, Math.floor(frontText.length * 0.9));
    }
    
    // Get the back chunk
    let backText = text.slice(Math.floor(text.length * (1 - backRatio)));
    while (tokenCounter.countTokens(backText, model as any).tokens > backTokens) {
  ...
  private emergencyTruncate(text: string, maxTokens: number): string {
    const { tokens } = tokenCounter.countTokens(text, this.options.model as any);
    ...
    while (low < high) {
      ...
      const chunkTokens = tokenCounter.countTokens(chunk, this.options.model as any).tokens;
...
export const templates = {
   ...
   */
  crewOrchestration: createTemplate(
    '# Crew Orchestration\n\n' +
    '## Crew Objective\n' +
    '{{crewObjective}}\n\n' +
    '## Available Agents\n' +
    ...
    'and provide a plan for completing the crew objective.',
    { maxTokens: 3000 }
  )
};
